...
void ItemUpdater::onInterfacesAdded(sdbusplus::message_t& msg)
{
    sdbusplus::message::object_path objPath;
    std::map<std::string,
    ...
    sleep(1);
    auto service = utils::getService(bus, path.c_str(), ITEM_IFACE);
    psuStatusMap[path].present = utils::getProperty<bool>(
        bus, service.c_str(), path.c_str(), ITEM_IFACE, PRESENT);

    service = utils::getService(bus, path.c_str(), ASSET_IFACE);
    psuStatusMap[path].model = utils::getProperty<std::string>(
        bus, service.c_str(), path.c_str(), ASSET_IFACE, MODEL);

    auto version = utils::getVersion(path);
    if (psuStatusMap[path].present && !version.empty())
    {
        processPSUImage();
        processStoredImage();
        syncToLatestImage();
